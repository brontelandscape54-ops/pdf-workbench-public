...
![2ページ目を分割なしの1領域とした画面](images/02-one-region-page2.jpg)
...
**検証状況：** 添付画像は分割編集とBundle書き出しの実行例です。外部OCRソフトを通した復元結果の画面、OCRソフト名・バージョン、文字情報保持の実機検証結果は含まれていません。元のサンプルが検索可能であることだけを根拠に、OCR結果の保持を検証済みとは扱いません。

## 6. 自動分割を試す場合（任意）

「自動分割」タブでは、余白を解析して分割候補を作れます。以下はサンプルの**左右2領域を自動判定した例**です。手動で4領域にする手順の代わりとして必ず同じ結果が得られるわけではありません。

![自動分割タブで左右2領域を自動判定した例](images/04-auto-split-tab.jpg)

自動判定が細かくなりすぎる場合もあります。次は同じサンプルが**8領域になった例**で、赤い分割線が文章中を横切っています。このままOCR用PDFを書き出さず、Undo・領域の統合・分割線の調整などで修正してください。

![本文中まで細かく分割された8領域の例](images/05-auto-split-too-many.jpg)
